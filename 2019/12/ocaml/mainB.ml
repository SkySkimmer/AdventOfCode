...
let moons =
  ...
  let rec aux acc =
    match
      Scanf.bscanf input "<x=%d, y=%d, z=%d>\n" (fun x y z -> [| x; y; z |])
    with
    | exception _ -> acc
    | v -> aux (v :: acc)
  in
...
let prpoint ch v = Printf.fprintf ch "<x=%d, y=%d, z=%d>\n" v.(0) v.(1) v.(2)
...
let step_gravitate coord =
  for i = 0 to Array.length moons - 1 do
    ...
    for j = i + 1 to Array.length moons - 1 do
      ...
      speeds.(i).(coord) <-
        speeds.(i).(coord) + sign (othermoon.(coord) - moon.(coord));
      speeds.(j).(coord) <-
        speeds.(j).(coord) + sign (moon.(coord) - othermoon.(coord))
    done
  done

let step_move coord =
  Array.iter2
    (fun moon speed -> moon.(coord) <- moon.(coord) + speed.(coord))
    moons speeds

let step coord =
  step_gravitate coord;
  step_move coord
...
let rec find_repeat coord seen i =
  ...
  | None ->
    step coord;
    find_repeat coord seen (i + 1)

let repeats =
  Array.init
    (Array.length moons.(0))
    (fun coord ->
...
let lcm a b =
  if a = 0 then b (* <- we use 0 as the start value for the fold *)
  else
    let g = gcd a b in
    a * b / g
