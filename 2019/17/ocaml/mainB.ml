...
let rec read_line state r i =
  ...
  | HaveOutput o ->
    ...
    | '\n' ->
      abort_unless
        (i = Array.length r)
        "bad output (non newline at end of line)\n"
    | _ ->
...
let start =
  ...
  try
    Array.iteri
      (fun i line ->
        Array.iteri
          (fun j -> function '^' -> raise (Found (i, j)) | _ -> ())
          line)
      lines;
    abort "start not found\n"
  with Found r -> r
...
let safe_get (i, j) = try Some lines.(i).(j) with Invalid_argument _ -> None

let safe_scaffold p =
  match safe_get p with None -> false | Some '#' -> true | Some _ -> false
...
let show_compressed = function L -> "L" | R -> "R" | C i -> string_of_int i
...
let cpath =
  match compress [] 0 path with
  | C 0 :: rest -> rest
  | _ -> abort "unexpected compression\n"

let () =
  if debugging >= 1 then Format.printf "%a\n" (prlist pp_compressed "") cpath
...
let () =
  abort_unless
    (List.for_all (fun s -> String.length s <= 20) inputs)
    "too long input"
...
let rec feed_input state s i =
  ...
  | HaveOutput _o ->
    (* it prints the map and prompts, ignore them *) feed_input state s i
  | WaitInput k ->
    let k c = k (int_of_char c) in
    if i = String.length s then k '\n'
    else (
      k s.[i];
      feed_input state s (i + 1))

let rec feed_inputs state = function
  | [] -> ()
  | s :: rest ->
    feed_input state s 0;
    feed_inputs state rest
