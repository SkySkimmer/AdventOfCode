...
let compare_bits a b =
  if imply_bits a b then -1
  else if imply_bits b a then 1
  else 0
...
let search p =
  ...
  let rec aux () =
    ...
    | w ->
      ...
      if visited.(idx w.point) |> List.exists (fun keys' -> imply_bits w.keys keys') then aux ()
      else begin
        ...
        let v' = w.keys ::
                 (List.filter (fun keys' -> not (imply_bits keys' w.keys)) visited.(idx w.point))
        in
        visited.(idx w.point) <- v';

        let keys, missing = match get w.point with
          | Empty (Some (Key k)) ->
        ...
        if missing = 0 then w.cost
          ...
          List.iter (fun p -> Queue.add {point=p;keys;missing;cost=w.cost+1} work) neighbours;
          aux ()
        end
      end
  in
