...
let () = neighbours start |> List.iter (fun p -> tiles.(idx p) <- Wall)
      ...
      if visited.(idx p) then aux ()
        ...
        List.iter
          (fun p ->
            ...
            | Wall -> ()
            | Empty (Some (Door d)) ->
              Queue.add (p, cost + 1, setbit doors d) work
...
let ( >>= ) a f = List.concat (List.rev_map f a)
...
let possible_moves bots keys =
  ...
  let bot = bots.(i) in
  dists.(bot)
  |> List.filter (fun (target, (_, doors)) ->
...
let rec mem ((bots, keys) as x) = function
  ...
  | Node { l; v = bots', keys'; r; _ } ->
    ...
    if c < 0 then mem x l
      ...
      let c = compare keys keys' in
      if c < 0 then mem x l else mem x r

let mem x (set : PSet.t) = mem x (Obj.magic set)

let insert bots keys points =
  (bots, keys)
  :: List.filter
       (fun (bots', keys') -> not (bots = bots' && imply_bits keys' keys))
       points

let cost =
  let work =
    ref (IntMap.singleton 0 [ (Array.init 4 (fun i -> i + nkeys), 0) ])
  in
  ...
  let rec aux () =
    ...
    | Some (cost, points) ->
      ...
      points
      |> List.iter (fun ((bots, keys) as p) ->
             if mem p !visited then ()
             else begin
               visited := PSet.add p !visited;
               if keys = allkeys then raise (Found cost);
               possible_moves bots keys
               |> List.iter (fun (bots, keys, mcost) ->
                      let cost = cost + mcost in
                      work :=
                        !work
                        |> IntMap.update cost (function
                             | None -> Some [ (bots, keys) ]
