...
(* [egcd a b] returns [g,x,y] such that [a*x + b*y = g] and [g] is the
   gcd of [a] and [b]. *)
let rec egcd a b = if Big_int.eq_big_int a z then (b,z,u)
  else
...
let reverse ~len = function
  ...
  | Deal n ->
    ...
    abort_unless (Big_int.eq_big_int g u && Big_int.le_big_int x blen)
...
b2' = b1 * a2 + b2
*)
let combine (a1,b1) (a2,b2) =
  (a1 * a2) mod blen, (b1 * a2 + b2) mod blen

let a,b =
  List.fold_left (fun x1 a ->
      let a,b = act a in
