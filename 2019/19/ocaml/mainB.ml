...
let get x y =
  ...
  | Done | HaveOutput _ -> abort "bad1"
  | WaitInput k ->
    k x;
    (match run state with
    | Done | HaveOutput _ -> abort "bad2"
    | WaitInput k ->
      k y;
      (match run state with
      | Done | WaitInput _ -> abort "bad3"
      | HaveOutput o -> o))

(* safe wrapper so I don't have to worry about sign *)
let get x y = x >= 0 && y >= 0 && get x y = 1

let rec findbeam x y = if get x y then x else findbeam (x + 1) y

let rec findwidth x width y =
  if get (x + width) y then findwidth x (width + 1) y else width
...
let rec checkback x i = function
  | [] -> false
  | (startx, width) :: rest ->
    if i > 1 then checkback x (i - 1) rest else startx + width >= x + goalw

let rec aux x width y =
  let x = findbeam x y in
  let width = findwidth x width y in
  debug 2 "found x=%d,width=%d,y=%d\n" x width y;
  ...
  lines := (x, width) :: !lines;
  if width >= goalw && checkback x goalh !lines then (x, y - goalh + 1)
  else aux x width (y + 1)
