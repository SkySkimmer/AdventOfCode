...
let rec get_output state =
  ...
  | Done -> []
  | WaitInput _ ->
    abort "no input to give!\n"
  | HaveOutput o -> o :: get_output state

module PMap = Map.Make(struct type t = int * int let compare = compare end)
...
let rec interp acc = function
  | [] -> acc
  | x :: y :: c :: rest ->
    let acc = PMap.union (fun _ _ _ -> (abort "double write!\n" : _ option))
        acc (PMap.singleton (x,y) (parse_tile c))
    in
    interp acc rest
...
let count = PMap.fold (fun _ c acc -> if c = Block then acc+1 else acc) points 0
