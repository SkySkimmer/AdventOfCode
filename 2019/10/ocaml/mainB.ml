...
let () =
  debug "[%a] =\n[%a]\n" (prlist prvec "; ") (List.rev testpos)
    (prlist prvec "; ") testsort

let () =
  abort_unless
    (try List.for_all2 veq (List.rev testpos) testsort
     with Invalid_argument _ -> false)
    "bad compare_directions"

let astra_view =
  astra_view |> List.sort (fun (_, a) (_, b) -> compare_directions a b)

let rec packetize acc cur = function
  | [] -> List.rev (cur :: acc)
  | x :: rest ->
    if same_direction (snd (List.hd cur)) (snd x) then
      packetize acc (x :: cur) rest
...
let astra_view = astra_view |> List.map (List.sort compare_norm)
