...
type state = { mem : int array; mutable pc : int;
               mutable input : int list; mutable output : int list; }
...
let set ~mode v = match mode with
  ...
  | Position -> state.mem.(state.mem.(state.pc)) <- v; incr_pc ()
...
let rec all_runs i data =
  if i = max_coeff+1 then
    List.map (fun (coeffs,history,output) ->
        abort_unless (coeffs = []) "unexpected remaining coeffs";
        history,output)
      data
  else
    let data = List.flatten (List.map (fun (coeffs,history,output) ->
        List.map (fun coeff ->
...
let max = match all_runs with
  | [] -> abort "missing runs"
  | (_,output) :: rest ->
    find_max output rest
