...
module Point : sig
  ...
  let parse s =
    ...
    points := SMap.update s (function
        | None -> incr npoints; rpoints := IMap.add !r s !rpoints; Some !r;
        | Some n as v -> r:=n; v)
        !points;
    !r
...
let rec parse ch acc =
  ...
  | l ->
    let acc = match String.split_on_char ')' l with
      | [a;b] ->
...
let count, counts = PMap.fold (fun p _ (count,counts) ->
