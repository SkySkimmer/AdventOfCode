...
type gamestate = { tiles : tile PMap.t; self : Point.t; minx : int; miny : int }
...
let get_move tiles self move =
  ...
  match
    (f move, f (turn move), f (turn (turn move)), f (turn (turn (turn move))))
  with
  | None, _, _, _ -> move
...
let push_neighbours (x, y) cost =
  List.iter
    (fun p -> Queue.add (p, cost + 1) work)
    [ (x, y - 1); (x, y + 1); (x - 1, y); (x + 1, y) ]

let try_find tiles =
  incr frame;
  if !frame mod 65536 = 0 then
    let rec find () =
...
let update_game game move tile =
  ...
  debug "at %d,%d got %s moved %s\n" (fst self) (snd self) (pr_tile tile)
    (pr_move move);
...
let rec search state game move =
  ...
  | HaveOutput o ->
    (match update_game game move (parse_tile o) with
    | `Done res -> res
    | `UnknownRemains (game, move) -> search state game move)
