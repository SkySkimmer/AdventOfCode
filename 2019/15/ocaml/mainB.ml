...
let com_of_dir = function Up -> 1 | Down -> 2 | Left -> 3 | Right -> 4
...
let pr_tile = function Empty -> "." | Wall -> "W" | Goal -> "G"

let print1 ~self = function
  | Empty -> print_char (if self then 'X' else '.')
  | Wall ->
    abort_unless (not self) "self can't be on wall";
    print_char 'W'
    ...
    let _, _ =
      PMap.fold
        (fun pos' tile pos ->
          ...
          if pos' = (0, 0) then print_char 'O'
          else print1 ~self:(pos' = game.self) tile;
          pos')
        game.tiles (game.minx, game.miny)
    in
    print_string "\n";
    flush stdout
  end
...
let turn = function Up -> Left | Down -> Right | Left -> Down | Right -> Up

let pr_move = function Up -> "U" | Down -> "D" | Left -> "L" | Right -> "R"
...
let make_path game target =
  ...
  let rec find visited =
    ...
    if visited == visited' then find visited
      ...
      match PMap.find_opt p game.tiles with
      | None | Some Wall -> find visited'
      | Some _ ->
        List.iter
          (fun (p, d) -> Queue.add (p, d :: path) work)
          (neighbours_dir p);
        find visited'
  in
  find PSet.empty
...
let rec prlist pr sep ch = function
  ...
  | x :: rest ->
    pr ch x;
    Printf.fprintf ch sep;
    prlist pr sep ch rest

let pr_path ch path =
  prlist (fun ch d -> Printf.fprintf ch "%s" (pr_move d)) "" ch path
...
let rec search state game =
  ...
  | WaitInput k ->
    ...
    search state game
  | HaveOutput o ->
    (match update_game game (parse_tile o) with
    | `Done res ->
      draw_game game;
      res
    | `Continue game ->
...
let start_game =
  {
    ...
    work = [ (0, 0) ];
    path = [ Up ];
    (* start by fake Up movement from (0,1) *)
    minx = 0;
    miny = 0;
  }

let start_game =
  match update_game start_game Empty with
  | `Continue game -> game
  | `Done _ -> assert false
...
let goal =
  match game.goal with None -> abort "no goal found\n" | Some goal -> goal
