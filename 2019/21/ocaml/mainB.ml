...
(* if there is ground where we would land and a hole in between then
   jump, also make sure we have a next possible action (E or H) *)
let input = "NOT A J
NOT J J
AND B J
AND C J
...
let rec aux state i =
  ...
  | Done -> ()
  | HaveOutput o ->
    print_string (string_of_output o); aux state i
  | WaitInput k ->
    if i = String.length input then abort "no input to give\n";
    k (int_of_char (String.get input i)); aux state (i+1)
