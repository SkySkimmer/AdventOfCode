...
let parse_line len l =
  Array.init len (fun j -> if j < String.length l then parse_tile (String.get l j) else Void)
...
let get_lines ch =
  ...
  let maxlen = if String.get l3 (len3-1) = '#' then len3+2 else len3 in
...
let portals, start, goal =
  ...
  let goal = ref 0 in
  let portals =
    PortMap.fold (fun pname points portals ->
...
let is_outer p =
  let i,j = unidx p in
  i = 2 || j = 2 || i = nlines - 3 || j = linelen - 3

let neighbours (p,layer) =
  let n = List.map (fun d -> p+d,layer) dirs in
  let n = List.filter (fun (p,_) -> get p = Empty) n in
  let outer = is_outer p in
  if outer && layer = 0 then n
  else
    match PMap.find_opt p portals with
    | None -> n
    | Some p' -> (if outer then p',layer-1 else p',layer+1) :: n
