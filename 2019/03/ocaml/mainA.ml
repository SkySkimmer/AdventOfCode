...
let parse_move m = Scanf.sscanf m "%c%d" (fun d n -> (parse_dir d, n))
...
let rec make_segments start acc = function
  | [] -> List.rev acc
  | m :: rest -> make_segments (move start m) (make_segment start m :: acc) rest
...
let intersect (d, n, (x, y)) (d', n', (x', y')) =
  if d = d' then None (* assume no parallel intersections *)
    ...
    if x' <= x && x <= x' + n' && y <= y' && y' <= y + n then Some (x, y')
    else None
    ...
    (fun acc s1 ->
      List.fold_left
        (fun acc s2 ->
          ...
          | Some a, Some b -> if norm a < norm b then Some a else Some b)
        acc s2)
    None s1

let res = match res with Some res -> res | None -> assert false
