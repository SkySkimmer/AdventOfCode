...
let compare cmp v1 v2 =
  if v1 == v2 then 0
    ...
    let c = compare len (Array.length v2) in
    if c <> 0 then c
    else
      let rec loop i =
        if i < 0 then 0
...
let equal_norefl cmp t1 t2 =
  ...
  if not (len = Array.length t2) then false
  else
    let rec aux i =
      if i < 0 then true
      else
...
let equal cmp t1 t2 = if t1 == t2 then true else equal_norefl cmp t1 t2
...
let for_all3 f v1 v2 v3 =
  let rec allrec = function
    | -1 -> true
    | n ->
      let ans = f (uget v1 n) (uget v2 n) (uget v3 n) in
      ans && allrec (n - 1)
  in
  let lv1 = Array.length v1 in
...
let for_all4 f v1 v2 v3 v4 =
  let rec allrec = function
    | -1 -> true
    | n ->
      let ans = f (uget v1 n) (uget v2 n) (uget v3 n) (uget v4 n) in
      ans && allrec (n - 1)
  in
  let lv1 = Array.length v1 in
  lv1 = Array.length v2
...
let for_all_i f i v =
  ...
  let rec allrec i n = n = len || (f i (uget v n) && allrec (i + 1) (n + 1)) in
  allrec i 0
...
let findi (pred : int -> 'a -> bool) (arr : 'a array) : int option =
  try
    for i = 0 to Array.length arr - 1 do
      if pred i (uget arr i) then raise (Found i)
    done;
    None
  with Found i -> Some i

let hd v = match Array.length v with 0 -> failwith "Array.hd" | _ -> uget v 0
...
let last v =
  match Array.length v with 0 -> failwith "Array.last" | n -> uget v (pred n)
...
let fold_right2 f v1 v2 a =
  ...
  let rec fold a n =
    if n = 0 then a
      ...
      let k = n - 1 in
      fold (f (uget v1 k) (uget v2 k) a) k
  in
...
let fold_right3 f v1 v2 v3 a =
  ...
  let rec fold a n =
    if n = 0 then a
      ...
      let k = n - 1 in
      fold (f (uget v1 k) (uget v2 k) (uget v3 k) a) k
  in
  if Array.length v2 <> lv1 || Array.length v3 <> lv1 then
    invalid_arg "Array.fold_right3";
  fold a lv1
...
let fold_left4 f a v1 v2 v3 v4 =
  ...
  in
  if Array.length v2 <> lv1 || Array.length v3 <> lv1 || Array.length v4 <> lv1
  then invalid_arg "Array.fold_left4";
  fold a 0

let fold_left_from n f a v =
  let len = Array.length v in
  let () = if n < 0 then invalid_arg "Array.fold_left_from" in
  let rec fold a n = if n >= len then a else fold (f a (uget v n)) (succ n) in
  fold a n
...
let map2_i f v1 v2 =
  ...
  let () = if not (len1 = len2) then invalid_arg "Array.map2" in
  if len1 = 0 then [||]
  else
...
let map3 f v1 v2 v3 =
  let len1 = Array.length v1 in
  let () =
    if len1 <> Array.length v2 || len1 <> Array.length v3 then
      invalid_arg "Array.map3"
  in
  if len1 = 0 then [||]
  else
...
let map3_i f v1 v2 v3 =
  ...
  let () =
    if not (len1 = len2 && len1 = len3) then invalid_arg "Array.map3_i"
  in
  if len1 = 0 then [||]
  else
...
let map_left f a =
  (* Ocaml does not guarantee Array.map is LR *)
  let l = Array.length a in
  (* (even if so), then we rewrite it *)
  if l = 0 then [||]
  else
...
let iter2_i f v1 v2 =
  let len1 = Array.length v1 in
  let len2 = Array.length v2 in
  let () = if not (len2 = len1) then invalid_arg "Array.iter2" in
  for i = 0 to len1 - 1 do
    f i (uget v1 i) (uget v2 i)
  done
...
let distinct v =
  ...
  try
    Array.iter
      (fun x ->
        if Hashtbl.mem visited x then raise Exit else Hashtbl.add visited x x)
      v;
    true
  with Exit -> false
...
let rev_to_list a =
  let rec tolist i res =
    if i >= Array.length a then res else tolist (i + 1) (uget a i :: res)
  in
  tolist 0 []
...
module Smart = struct
  ...
  let map2 f aux_ar ar =
    ...
    let () = if not (len = aux_len) then invalid_arg "Array.Smart.map2" in
    let i = ref 0 in
    let break = ref true in
    let temp = ref None in
    while !break && !i < len do
  ...
  let fold_left2_map f accu aux_ar ar =
    ...
    let aux_len = Array.length aux_ar in
    let () =
      if not (len = aux_len) then invalid_arg "Array.Smart.fold_left2_map"
    in
    let i = ref 0 in
    let break = ref true in
    let r = ref accu in
...
module Fun1 = struct
  ...
  let iter2 f arg v1 v2 =
    let len1 = Array.length v1 in
    let len2 = Array.length v2 in
    let () = if not (len2 = len1) then invalid_arg "Array.Fun1.iter2" in
